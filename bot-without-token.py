...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if ".dcinside.com/" in  message.content: # 채팅에서 디씨 링크 여부 참조
            ...
            embed=discord.Embed(title=doc.title, url=f"https://m.dcinside.com/board/{gallname}/{postnum}", description=f"텍스트 {len(doc.contents)}자 이미지 {image_count}개", color=0x357df2)
